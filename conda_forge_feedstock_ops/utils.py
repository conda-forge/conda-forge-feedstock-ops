...
class TimeoutTimer:
    ...
    def raise_for_timeout(self):
        if self.elapsed > self.timeout:
            raise TimeoutTimerException(f"timeout out for {self.name}")
...
def convert_spec_to_conda_build(myspec):
    ...
    if build and not version:
        raise RuntimeError(f"spec '{myspec}' has build but not version!")
...
def apply_pins(reqs, host_req, build_req, outnames, m):
    ...
    for dep in reqs:
        try:
            ...
            print_critical(
                f"Failed to apply pin for {dep}, falling back to req: {repr(e)}",
            )
    ...
    for req in reqs:
        if "pin_compatible(" in req:
            if not req.startswith("pin_compatible("):
                raise ValueError(f"Very odd pinning: {req}!")
            parts = req.rsplit(")", 1)
            if len(parts) == 2:
                build = parts[1].strip()
            else:
            ...
            if name not in host_lookup or not host_lookup[name]:
                if strict:
                    if name not in host_lookup:
                        raise ValueError(
                            f"Very odd pinning: {req}! Package {name} not found in host {host_lookup!r}!"
                        )
                    else:
                        raise ValueError(
                            f"Very odd pinning: {req}! Package found in host "
                            f"but no version {host_lookup[name]!r}!"
                        )
                else:
                    if build:
            ...
            if len(host_lookup[name]) > 1:
                ...
                if build and "exact=true" in req.lower():
                    raise ValueError(
                        ...
                        f"pin_compatible with exact=True! {req!r}"
                    )
            else:
                host_build = ""
