...
from conda_forge_feedstock_ops.url_transforms import gen_transformed_urls
...
def _try_pypi_api(url_tmpl: str, context: MutableMapping, hash_type: str, cmeta: Any):
    ...
    if hasattr(cmeta, "meta"):
        ...
        for output in cmeta.get("outputs", []):
            package_name = _get_keys_default(
...
def _try_to_update_version(cmeta: Any, src, hash_type: str):
    ...
    if ha is None:
        errors.add(f"invalid hash type {hash_type}")
        return False, errors
        ...
        for var in jinja2_var_set:
            ...
            if len(possible_keys) == 0:
                if var == "cran_mirror":
                    ...
                    errors.add(
                        f"missing jinja2 variable '{var}' for selector '{selector}'"
                    )
        ...
        if isinstance(src[url_key], collections.abc.MutableSequence):
            for url_ind, url_tmpl in enumerate(src[url_key]):
                ...
                if new_hash is not None:
                    ...
                    errors.add(f"could not hash URL template '{url_tmpl}'")
        else:
            new_url_tmpl, new_hash = _get_new_url_tmpl_and_hash(
                src[url_key],
                context,
            ...
            if new_hash is None:
                errors.add(f"could not hash URL template '{src[url_key]}'")
        ...
        if new_hash is not None:
            ...
            if _replaced_hash:
                ...
                errors.add(
                    ...
                    f"for URL template '{new_url_tmpl}'",
                )
...
def update_version(feedstock_dir, version, hash_type="sha256", use_container=None):
    ...
    if should_use_container(use_container=use_container):
        return _update_version_containerized(
            feedstock_dir,
            version,
            hash_type,
        )
    else:
        return _update_version_local(
            feedstock_dir,
            version,
            hash_type,
        )
...
def _update_version_local(feedstock_dir, version, hash_type) -> tuple[bool, set]:
...
def _update_version_containerized(feedstock_dir, version, hash_type):
    ...
    for source in rendered_sources:
        ...
        for url, template in urls:
            ...
            if new_hash is not None:
                ...
                errors.add(f"could not hash URL template '{cb_template}'")
...
def _update_version_v0_local(
    ...
    if not isinstance(version, str):
        errors.add(
            f"the version '{version}' is not a string and must be for the bot",
        )
        logger.critical(
